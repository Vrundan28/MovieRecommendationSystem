...
def get_index_from_title(title):
    # print(title)
    # print(dataset["movieTitle"])
    return dataset[dataset.movieTitle == title]["movieId"].values[0]


def getRecommendationsFunc(movieId):
    ...
    def combine_feature(row):
        return str(row['movieTitle'])+" "+str(row['movieCast'])+" "+str(row['movieGenre'])+" "+str(row['movieDirector'])
    ...
    # print(similarity)
    # movie_index = 1729
    similar_movies = list(enumerate(similarity[movieId-1]))
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                customUser = CustomUser(user_id=user.id,isFilled=1)
                movie = Movie.objects.all()
                row=[]
                user.save()
                customUser.save()
                cur_user = CustomUser.objects.get(user_id=customUser.user_id)
                print(customUser.id)
                row.append(cur_user.id)
...
def get_liked_movies_of_user(request, id):
    ...
    if x != 0:
        ...
        for i in range(0, min(10,len(liked_movies))):
            current_movie = liked_movies[i].movie.movieTitle
            recommendations_for_current_movie = []
            recommended_movie_tuples = getRecommendationsFunc(liked_movies[i].movie.movieId)
            cnt = 0
...
def getrecommendations(request, id):
    ...
    # print(movie)
    current_movie = movie.movieId
    recommended_movie_tuples = getRecommendationsFunc(current_movie)
